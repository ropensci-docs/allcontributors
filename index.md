...
1.  Works automatically, by calling
    [`add_contributors()`](https://docs.ropensci.org/allcontributors/reference/add_contributors.md)
    at any time to add or update contributor acknowledgements.
...
command will then install the development version. Alternatively, any of
the following options may be used for those who prefer not to use
GitHub:
...
`"GITHUB"` (either via
[`Sys.setenv()`](https://rdrr.io/r/base/Sys.setenv.html), or as an
equivalent entry in a file `~/.Renviron`).
...
[`add_contributors()`](https://docs.ropensci.org/allcontributors/reference/add_contributors.md)
function. There’s even an `open_issue` parameter that will automatically
open or update a github issue on your repository so that contributors
will be pinged about them being added to your list of contributors.
